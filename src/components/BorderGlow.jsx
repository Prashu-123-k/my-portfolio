...
const BorderGlow = ({
  ...
  useEffect(() => {
    ...
    const startId = requestAnimationFrame(() => setIsPlaying(true));
    const stopId = window.setTimeout(() => setIsPlaying(false), 900);
  ...
  const revealStrength = isHovered ? edgeProximity : isPlaying ? 1 : 0;
  const borderOpacity = isVisible
    ? Math.max(0, (revealStrength * 100 - colorSensitivity) / (100 - colorSensitivity))
    : 0;
  const glowOpacity = isVisible
    ? Math.max(0, (revealStrength * 100 - edgeSensitivity) / (100 - edgeSensitivity))
    : 0;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col relative overflow-hidden z-1">{children}</div>
    </div>
  );
};
